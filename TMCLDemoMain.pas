...
type
  TFormMain = class(TForm)
    ...
    resetTmcl: TButton;
    reziprok_checkbox: TCheckBox;
    movement_groupbox: TGroupBox;
    directionHint_label: TLabel;
    directionHint_label2: TLabel;
    directionHint_label3: TLabel;
    speedhint_label: TLabel;
    stxtMicrosteps: TStaticText;
    ...
    procedure updateGraph(position: Integer);
    ...
    procedure resetTmclClick(Sender: TObject);
    procedure reziprok_checkboxClick(Sender: TObject);
...
// Globale Variablen
  public
    ...
    function OpenComDevice(comDevice: ShortString) : boolean;
...
begin
  ...
  begin
    ...
    StaticDefault.Caption:=IntToStr((TMCLReply[4] shl 24) or (TMCLReply[5] shl 16) or (TMCLReply[6] shl 8) or TMCLReply[7]);



    // In StaticValue kommen nur noch die Werte für die Position, damit es bei der Berechnung der Durchläufe keine Fehler gibt.
    if (TMCLReply[3] = 6) then
    begin

    //if not 
        StaticValue.Caption:=IntToStr((TMCLReply[4] shl 24) or (TMCLReply[5] shl 16) or (TMCLReply[6] shl 8) or TMCLReply[7]);
    
    // if strtoint(StaticValue.Caption) = 0 then
    //   updateGraph(strtoint(stxtPosition.Caption));

   // begin
        //stxtPosition.Caption := inttostr(round(0.1125 * strtoint(StaticValue.Caption)));
       // stxtMicrosteps.Caption := StaticValue.Caption;
       // updateGraph(strtoint(stxtPosition.Caption));
    //  end;
    end;
    ...
    if ((TMCLReply[3] > 0) AND (TMCLReply[3] < 10)) then
      sgdTMCL.Cells[1,TMCLReply[3]] := IntToStr((TMCLReply[4] shl 24) or (TMCLReply[5] shl 16) or (TMCLReply[6] shl 8) or TMCLReply[7]);
  end;
   end;
...
begin
  ...
  begin
    ...
    BtnClose.Enabled:=true;
     ...
     //speed.Value :=(TMCLReply[4] shl 24) or (TMCLReply[5] shl 16) or (TMCLReply[6] shl 8) or TMCLReply[7];
...
function TFormMain.OpenComDevice(comDevice: ShortString) : boolean;
begin
  ...
  //when RS485 is used.
  //Sending and receiving might not work if the flow control is set to any other option.
  Comm32.DeviceName:=TrimRight(LeftStr(comDevice,Pos('|',comDevice)-1));
  Comm32.Open;
  if Comm32.Enabled then    
  begin
    BtnOpen.Enabled:=false;
...
var
  ...
  if MessageDlg('Spannbacken lösen! Feile darf nicht klemmen!', mtConfirmation, [mbOK, mbCancel], 0) = mrCancel then
    begin
       startButton.Enabled := TRUE;
       exit;
      //fehler := TRUE;
  ...
  // if not fehler then
  // Geschwindigkeit für die Fahrt auf ABS POS 0
 ...
 repeat
              ...
              // StaticValue auslesen und in Übersichtsmaske (stxtPosition) einblenden
              stxtPosition.Caption := inttostr(round(0.1125*strtoint(StaticValue.Caption)));
              stxtMicrosteps.Caption := StaticValue.Caption;
              ...
              // running ist false, wenn keine Strom mehr über die Feile fließt

            updateGraph(strtoint(stxtPosition.Caption));

          until (strtoint(StaticDefault.Caption)=1);
  ...
  if ((Data2 <= sedtGrenzwert.Value)) then
    begin
     MessageDlg('Messwert für Feilenspannung unter Grenzwert. Bitte Kontakte prüfen!', mtError, [mbOK], 0);
     //MessageDlg('Messwert für Feilenspannung unter Grenzwert. Bitte Kontakte prüfen!', mtConfirmation, [mbOK, mbCancel], 0) = mrOK);
     fehler := true;
     startButton.Enabled := TRUE;
     exit;
    end;
  ...
  // Alte Messungen aus der Tabelle löschen? BUG
  if MessageDlg('Alte Details löschen?', mtConfirmation, [mbOK, mbCancel], 0) = mrOK then
    begin
  ...
  richtung := true;
  first := FALSE;   // FALSE = erster Durchlauf, true=alle weiteren Durchläufe
  richtung_int := 1;  // richtung_int bestimmt später die Laufrichtung des Motors und geht als Faktor in MVP-Position ein.
  ...
  // Geschwindigkeit für die Fahrt festlegen
  ...
  if cbxRechts.Checked then
    ...
    begin
      ...
      repeat
        ...
        // StaticValue.Caption = Absolute zurückgelegte Microsteps (vom Motor aus: links = positiv, rechts = negativ)
        // 0.1125 = 360 / 3200 (Grad pro Microstep)
        stxtPosition.Caption := inttostr(round(0.1125 * strtoint(StaticValue.Caption)));
        stxtMicrosteps.Caption := StaticValue.Caption;

        count := -round( (strtoint(stxtPosition.Caption) / 360) + 0.5);
        counter.Caption := inttostr(count);
    ...
    begin
      ...
      if reziprok_checkbox.Checked = FALSE then
        ...
        repeat
         ...
         // if richtung then richtung_int := 1 else richtung_int := -1;
          ...
          //SendTMCLAndWait(1, 4, 1, 0, richtung_int*(pos_rechts - ord(first)*pos_links), false);
          if richtung then
            SendTMCLAndWait(1, 4, 1, 0, pos_rechts, false)
          else
            SendTMCLAndWait(1, 4, 1, 0, pos_links, false);
          ...
          if (richtung = FALSE AND first = TRUE) then count:= count+1;
  ...
  if cbxWarnton.Checked then
	...
	if cbxWarntonTimer.Checked then
		begin
			t1:=GetTickCount;
			while(Abs(GetTickCount-t1)<500) do ;
...
self.Left := 6;
  ...
  CbCOM.ItemIndex := 0;
  //Überschriften der Tabelle erstellen
  sgdDaten.Cells[0,0] := 'Feile';
  sgdDaten.Cells[1,0] := 'ISO';
  sgdDaten.Cells[2,0] := 'Länge';
  ...
  // Richtigen Com Port finden. Testet von hinten nach vorne, da erster Com Port normalerweise nie das Gerät ist
  for i:=0 to CbCOM.Items.Count-1 do begin
    
    if OpenComDevice(CbCOM.Items[CbCOM.Items.Count-1 -i]) then
   ...
   SendTMCLWaitResponse(1, 6, 1, 0, 0, true);
    ...
    updateGraph(strtoint(stxtPosition.Caption));
      ...
      CbCOM.ItemIndex := CbCOM.Items.Count-1 -i;
      startButton.Enabled := TRUE;
      Break
...
var
  ...
  canwidth, canheight, j, zeilenzahl: integer;
begin
  ...
  // temporär zur Nachverfolgung der Mittelwerte   + ';' + inttostr(SendTMCLWaitResponse(1,6,1,0,0,true))
...
procedure TFormMain.updateGraph(position: Integer);

var
  canwidth, canheight: integer;
...
With Image1.Canvas do
      Begin
        MoveTo(round(canwidth/2),round(canheight/2));
        LineTo(round(canwidth/2+(canwidth/2-5)*-sin(DegToRad(position))),round(canheight/2+(canheight/2-5)*-cos(DegToRad(position))));
        pen.color:=clblack;
        Brush.Color:=clred;
      end;
...
begin
  if cbxRechts.Checked then
    begin
       ...
       reziprok_checkbox.Enabled := FALSE;
    end
  else
    begin
       //Wenn Dauerrechts angeklickt wird, werden die anderen Felder ausgegraut
       ...
       reziprok_checkbox.Enabled := TRUE;
    end;
end;
...
procedure TFormMain.resetTmclClick(Sender: TObject);
begin
    ...
    // SendTMCLWaitResponse(1, 6, 8, 0, 0, true);
...
procedure TFormMain.reziprok_checkboxClick(Sender: TObject);

begin

  if reziprok_checkbox.Checked then
    begin
      cbxLinks.Checked := FALSE;
      cbxLinks.Enabled := FALSE;
      cbxRechts.Checked := FALSE;
